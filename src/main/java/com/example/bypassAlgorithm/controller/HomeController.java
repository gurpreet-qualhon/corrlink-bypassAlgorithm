...
public class HomeController {
	private static final DateFormat portableDateFormatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ",
			Locale.US);
//
...
//	CourseService courseService;
	...
	@GetMapping("/fun")
	public String fun() {
		return "Welcome to spring boot java framework";
...
//		System.out.println("@@@@@@@@@@@");
	...
	private String loginWithData(@RequestBody UserAccount userAccount) {
		...
		if (user != null) {
			...
			return this.finalPayload(loginUser.getResponseBody(), loginUser.getStatus(),
					loginUser.getCustomAuthToken());
		} else {
	...
	private String genrateAuthToken(String login_token) {
		...
		try {
			...
			String base64encodedString = Base64.getEncoder()
					.encodeToString(instance.doFinal(sb.toString().getBytes("utf-8")));
